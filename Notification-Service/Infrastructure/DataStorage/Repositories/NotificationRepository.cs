...
namespace Notification_Service.Infrastructure.DataStorage.Repositories
{
    ...
    {
        public static ISpecification<Notification> SearchByStatus(string status) => Specification<Notification>.Create(x => x.Status == status);
        public static ISpecification<Notification> SearchByContentType(string contentType) => Specification<Notification>.Create(x => x.ContentType == contentType);
